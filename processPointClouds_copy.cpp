...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    if(inliers->indices.size()==0){
        std::cout<<"Couldn't extract the points from the point cloud"<<std::endl;
    }
    std::cout<<"Inside Segment Plane"<<std::endl;
    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    for(int i =0;i<cluster_indices.size();i++){
        ...
        std::vector<int> indc = cluster_indices[i].indices;
        for(int j=0;j<indc.size();j++){
            cloud_cluster->points.push_back(cloud->points[indc[i]]);
        }
